...
function toV2Path(location) {
  const pathname = location.pathname === '/' ? '/v2' : `/v2${location.pathname}`;
    ...
    <div className="app-shell flex h-screen overflow-hidden">
      ...
      <aside className={cn('glass-sidebar relative z-50 flex flex-col shrink-0 overflow-visible transition-all duration-200', collapsed ? 'w-16' : 'w-60')}>
        ...
        <div className={cn('py-4 border-t border-sidebar-border space-y-3', collapsed ? 'flex flex-col items-center px-2' : 'px-4')}>
          ...
          <Link
            to={toV2Path(location)}
            onClick={(event) => handleSidebarNavigation(event, toV2Path(location))}
            className={cn(
              'rounded-md border border-white/10 bg-white/5 text-center text-xs font-semibold text-sidebar-foreground/70 shadow-sm transition-colors hover:bg-white/10 hover:text-sidebar-foreground',
              collapsed ? 'h-8 w-10 px-0 py-2 text-[9px]' : 'w-full px-3 py-2'
            )}
            title="Open v2 workspace"
          >
            V2
          </Link>
          <button
